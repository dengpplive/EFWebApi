...
using YSL.Common.Utility.Mapping;
    ...
    public static class ObjectHelper
    {
        ...
        private static object GetValue(Type type, object value, object default_value, string datetime_format)
        {
            ...
            else if (type == typeof(Int32) || type == typeof(int) || type == typeof(Int16) || type == typeof(Int64))
            {
                ...
                if (iv == 0 && value.ToString().Trim().ToUpper() == "ON")
                {
                    iv = 1;
                }
                else if (iv == 0 && value.ToString().Trim().ToUpper() == "TRUE")
                {
                    iv = 1;
                }
                v = iv;
            ...
            else if (type == typeof(Boolean))
            {
                ...
                if (!Boolean.TryParse(value.ToString(), out bv))
                {
                    ...
                    if (!bv)
                    {
                        bv = value.ToString().Trim().ToUpper() == "ON";
                    }
                }
                v = bv;
            }
            else if (type == typeof(DateTime))
            {
                ...
                else if (!string.IsNullOrEmpty(value.ToString().Trim()))
                {
                    ...
                    try
                    {
                        dt = DateTime.ParseExact(value.ToString(), datetime_format, System.Globalization.DateTimeFormatInfo.InvariantInfo);
                    }
                    catch { }
            ...
            {
                return Convert.ChangeType(value, default_value.GetType());
            }
            return v;
        }
        ...
        {
            List<Mapping.PropertyMappingInfo> pmis = GetProperties(typeof(T));
        ...
        public static void SetValue(object t, string column, object value)
        {
            SetValue(t, column, value, "dd/MM/yyyy");
        }

        public static void SetValue(object t, string column, object value, string datetime_format)
        {
            List<PropertyMappingInfo> pmis = GetProperties(t.GetType());
            for (int i = 0; i < pmis.Count; i++)
            {
                Type type = pmis[i].PropertyInfo.PropertyType;
                object default_value = pmis[i].PropertyInfo.GetValue(t, null);
        ...
        public static List<string> GetObjectFields(Type t)
        {
            ...
            foreach (Mapping.PropertyMappingInfo p in GetProperties(t))
            {
            ...
            where T : class, new()
        {
            ...
            if (piFrList.Count > 0)
            {
                foreach (Mapping.PropertyMappingInfo piTo in GetProperties(toType))
                {
