...
namespace QConnectSDK.Api
{
    public partial class RestApi
    {
        ...
        /// 解析accessToken和
        /// </summary>
        /// <param name="urlParams">类似于access_token=FE0489A65632DBBCAB52718890DF755F&expires_in=7776000</param>
        /// <returns></returns>
        public OAuthToken GetUserAccessToken(string urlParams)
        {
        ...
        public  void GetUserOpenIdAsync(string accessToken,Action<string> success, Action<QzoneException> failure)
        {
            var request = _requestHelper.CreateOpenIDRequest(accessToken);
    
            ExecuteAsync(request, response =>
            {
                var userLogin = GetUserOpenId(response.Content);
                success(userLogin);
            }, failure);
        }
        ...
        public void GetCurrentUserAsync(Action<User> success,Action<QzoneException> failure)
        {
            ...
            ExecuteAsync(request, response =>
            {
                ...
                var user = Deserialize<User>(content);
                success(user);
            }, failure);            
        }
        ...
        public void AddFeedsAsync(Action<QzoneBase> success, Action<QzoneException> failure, string title, string url, string comment = "", string summary = "", string images = "", string source = "", string type = "", string playurl = "", string nswb = "")
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
        ...
        /// <returns></returns>
         public void CheckPagefansAsync(Action<CheckPageResult> success, Action<QzoneException> failure, string qq)
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
        ...
        /// </summary>
        /// <returns></returns>
        public void GetCurrentUserListAlbumAsync(Action<UserAlbums> success, Action<QzoneException> failure)
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
        ...
        /// <returns></returns>
        public void AddAlbumAsync(Action<Album> success, Action<QzoneException> failure, string albumname, string albumdesc = "", int priv = 1, string question = "", string answer = "")
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
        ...
        /// <returns></returns>
        public void UploadPicAsync(Action<Picture> success, Action<QzoneException> failure, string photodesc, string title, string albumid, int? x, int? y, string picture, byte[] pictureData)
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
        ...
        /// <returns></returns>
        public void AddBlogAsync(Action<AddBlogResult> success, Action<QzoneException> failure, string title, string content)
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
        ...
        /// <returns></returns>
        public void AddTopicAsync(Action<QzoneBase> success, Action<QzoneException> failure, string richtype, string richval, string con, string lbs_nm, string lbs_x, string lbs_y, string third_source)
        {
            _restClient.Authenticator = new OAuthUriQueryParameterAuthenticator(context.AccessToken.OpenId, context.AccessToken.AccessToken, context.Config.GetAppKey());
